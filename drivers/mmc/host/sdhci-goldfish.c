...
static struct sdhci_ops sdhci_goldfish_ops = {
	...
	//.reset = sdhci_goldfish_reset,
	.set_clock = sdhci_goldfish_set_clock,
	//.set_bus_width = sdhci_goldfish_set_bus_width,
};
...
static int goldfish_mmc_probe(struct platform_device *pdev)
{
	...
	//mmc->ops = &goldfish_mmc_ops; //rida: will be overriden later in sdhci.c
	mmc->f_min = 400000;
	mmc->f_max = 24000000;
	...
	//GOLDFISH_MMC_WRITE(host, MMC_INT_ENABLE, MMC_STAT_END_OF_CMD | MMC_STAT_END_OF_DATA | MMC_STAT_STATE_CHANGE | MMC_STAT_CMD_TIMEOUT);
	
	//mmc_add_host(mmc);

	//struct sdhci_host host2;
	//host2.ioaddr = host->reg_base;
	//host2.ops = &sdhci_goldfish_ops;
	//host2.mmc = mmc;
	//host2.flags  |= SDHCI_USE_SDMA;

	//VERY Important in order to force "sdhci_do_get_cd" to return 1
	host->quirks |= SDHCI_QUIRK_BROKEN_CARD_DETECTION;
	//host->quirks |= SDHCI_QUIRK_NO_BUSY_IRQ; //for CMD6 - to avoid a timeout on this command
	//host->quirks |= SDHCI_QUIRK2_HOST_NO_CMD23;

	host->ops = &sdhci_goldfish_ops;
	ret = sdhci_add_host(host);
	printk("ret = sdhci_add_host(host); --> %d", ret);
	return ret;
